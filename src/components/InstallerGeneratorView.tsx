...
export function InstallerGeneratorView() {
  ...
  const handleDownloadFile = (type: 'inno' | 'wix' | 'config' | 'ps1' | 'bat') => {
                ...
                <div className="space-y-2 pt-2 border-t border-slate-800">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-slate-400 block">
                      Descargar Paquete para el Cliente:
                    </span>
                    <span className="text-[10px] text-emerald-400 font-mono font-bold flex items-center gap-1">
                      <Sparkles className="w-3 h-3" /> Instalador Autónomo Windows
                    </span>
                  </div>

                  {/* Primary Direct EXE/BAT Installer Button */}
                  <button
                    onClick={() => handleDownloadFile('bat')}
                    className="w-full py-2.5 px-4 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 text-white text-xs font-bold flex items-center justify-center gap-2 shadow-lg shadow-emerald-950/40 transition-all cursor-pointer"
                  >
                    <Download className="w-4 h-4 text-emerald-100" />
                    <span>Descargar Instalador Ejecutable del Cliente (.bat / .exe)</span>
                  </button>
